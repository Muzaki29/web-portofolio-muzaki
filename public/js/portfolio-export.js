/* Portfolio — fast PDF (text) & PowerPoint export */
(function () {
    'use strict';
    ...
    function setButtonsLoading(ids, loading) {
        const label = data.exportLoading || 'Preparing…';
        ids.forEach(id => {
            const btn = document.getElementById(id);
            if (!btn) return;
            if (loading) {
                if (!btn.dataset.exportLabel) {
                    btn.dataset.exportLabel = btn.innerHTML;
                }
                btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + label;
            } else if (btn.dataset.exportLabel) {
                btn.innerHTML = btn.dataset.exportLabel;
    ...
    function ensurePdfLibs() {
        if (!pdfReady) {
            pdfReady = (async () => {
                ...
                await waitFor(() => getJsPDF(), 10000);
            })();
        }
        return pdfReady;
    }

    function ensurePptLibs() {
        if (!pptReady) {
            pptReady = (async () => {
                if (typeof PptxGenJS === 'undefined') await loadScript(VENDOR.pptxgen);
                await waitFor(() => typeof PptxGenJS !== 'undefined', 10000);
            })();
        }
        return pptReady;
    }
    ...
    function preloadLibs() {
        ensurePdfLibs().catch(() => { pdfReady = null; });
        ensurePptLibs().catch(() => { pptReady = null; });
    }
    ...
    function exportPdfText(JsPDF) {
        ...
        addLine(
            (data.years || '4+') + ' ' + (data.yearsLabel || 'Tahun Pengalaman')
            + ' · 9+ ' + (data.projectsLabel || 'Proyek')
            + ' · 3 ' + (data.rolesLabel || 'Peran Utama'),
            10
        );
    ...
    async function exportPdf() {
        setBusy(true);
        setButtonsLoading(pdfBtns, true);
        try {
            await ensurePdfLibs();
            ...
            setButtonsLoading(pdfBtns, false);
            setBusy(false);
        }
    }
    ...
    function buildPptx() {
        ...
        slide.addText(
            (data.years || '4+') + ' ' + (data.yearsLabel || 'Years Experience')
            + '  ·  9+ ' + (data.projectsLabel || 'Projects')
            + '  ·  3 ' + (data.rolesLabel || 'Core Roles'),
            { x: 0.7, y: 2.85, w: 8.6, h: 0.5, fontSize: 12, color: maroon, bold: true }
        );
    ...
    async function exportPpt() {
        setBusy(true);
        setButtonsLoading(pptBtns, true);
        try {
            await ensurePptLibs();
            ...
            setButtonsLoading(pptBtns, false);
            setBusy(false);
        }
    }
    ...
    function init() {
        ...
        if ('requestIdleCallback' in window) {
            requestIdleCallback(preloadLibs, { timeout: 2500 });
        } else {
            setTimeout(preloadLibs, 1500);
        }
    }
